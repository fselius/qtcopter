...
class FlyToBaseFlow:
    """
    Fly to base site according to given, pre-configured coordinates.
    """
    ...
    def __call__(self, userdata, output):
        ...
        if not self.__sent_delta:
            ...
            rospy.loginfo('FlyToBaseFlow sending delta: %r' % ((x, y, z, theta),))
        ...
        try:
            ...
            rospy.logwarn('Could not lookupTransform in FlyToBaseFlow')
            time.sleep(0.1)
            return 'on way to base'
